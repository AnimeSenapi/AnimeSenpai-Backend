/**
 * Achievements Router V2 - Tier-based Achievement System
 * 
 * New tier-based achievement system where achievements have multiple tiers:
 * - Each achievement type has multiple tiers (1, 2, 3, etc.)
 * - Users progress through tiers as they complete requirements
 * - More scalable and organized than individual achievements
 */
...
export const achievementsV2Router = router({
  /**
    ...
    .query(async ({ ctx }) => {
      ...
      try {
        ...
        return {
          ...
          byCategory,
          unlockedTiersCount: totalTiersUnlocked,
          totalTiersAvailable: allAchievements.reduce((sum, a) => sum + a.tiers.length, 0),
          totalPoints
        }
  ...
  checkAndUnlock: protectedProcedure
    .mutation(async ({ ctx }) => {
      ...
      try {
        ...
        logger.info('Checked and unlocked achievements', logContext, { 
          userId: ctx.user.id,
  ...
  getStats: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      try {
        ...
        const totalPoints = await db.userAchievement.aggregate({
          where: { userId: ctx.user.id },
          _sum: {
            tier: {
              points: true
        ...
        return {
          ...
          totalPoints: totalPoints._sum.tier?.points || 0,
          categoryStats
        }
...
async function calculateUserProgress(userId: string): Promise<Record<string, number>> {
  const [
    ...
    // Genre diversity - get unique genres from user's anime
  ...
  genresWatched.forEach(item => {
    item.anime.genres.forEach(genre => {
      uniqueGenres.add(genre.slug)
    })
  })
